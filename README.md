...
Person AVercel (npm i -g vercel && vercel login)Person BBrowserbasePerson CStripe (test mode)Person DSupabase + burner X account
...
"We built an AI that autonomously launches products" → 15s
Live dashboard — agent decisions streaming → 30s
Browser replay — agent posting on X → 30s
Deployed site — it's real → 15s
Steering command — agent adapts → 15s
QR code — someone buys the product live → 15s

Cut list (if behind)

Reddit → just X
Steering → hardcode the run
Dashboard → show terminal logs
Never cut: Stripe + live site + browser replay
